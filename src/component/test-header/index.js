...
const CourseHeader = ({ isAdmin, title, instructor, thumnailImageUrl, containerStatus, setContainerStatus }) => {
  ...
  const handleClick = () => {
    if (isAdmin) router.push(`/test-ground/${router.query.id}`);
    else router.push(`/test-ground/${router.query.id}`);
  }
    ...
    <S.Container>
      ...
      <S.Course>
        ...
        <div>
          ...
          <div style={{ margin: "10px 0" }}>
            마지막 시간 : 2021년 09월 10일 13시 21분
          </div>
          <div style={{ margin: "8px 0" }}>
            {/* 인스턴스 상태 : {containerStatus} */}
          </div>
          <S.ContainerButtons>
            <S.Button onClick={handleClick}>테스트 응시</S.Button>
